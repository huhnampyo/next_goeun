      ...
      <style>
        {`
          ...
          .form-list select:focus {
            border-color: #4a90e2;
            outline: none;
            box-shadow: 0 0 3px rgba(74, 144, 226, .5);
          }
          ...
          (() => {
            ...
            function buildStepSettings(scenario, customer) {
              ...
              if (scenario.form === "APW" && scenario.biz === "법인사업자-대표자") {
                return [{ step_type: "05", step_name: "대표자", use_mail: true, use_sms: false, use_alimtalk: false, ...base, auth: { ...base.auth, method: "CORP_CERT" } }];
              }
              if (scenario.form === "APW" && scenario.biz === "법인사업자-대리인") {
                return [{ step_type: "05", step_name: "대리인", use_mail: true, use_sms: true, use_alimtalk: false, ...base, auth: { ...base.auth, method: "PASS" } }];
              }
              if (scenario.form === "ART" && scenario.biz === "개인-대표자" && scenario.contact === "비대면") {
                return [{ step_type: "05", step_name: "대표자", use_mail: false, use_sms: true, use_alimtalk: true, ...base, auth: { ...base.auth, method: "PASS" } }];
              }
              if (scenario.form === "ART" && scenario.biz === "개인-대표자" && scenario.contact === "대면") {
                return [{ step_type: "05", step_name: "대표자", use_mail: false, use_sms: false, use_alimtalk: true, ...base, auth: { ...base.auth, method: "NONE" } }];
              }
              ...
              if (scenario.form === "APW" && scenario.biz === "개인-대표자" && scenario.page === "3-7") {
                return [
                  { step_type: "05", step_name: "양수자", use_mail: true, use_sms: true, use_alimtalk: true, ...base, recipient: { id: customer.email_buyer || "", name: customer.name_buyer || "양수자" }, auth: { ...base.auth, method: "PASS" }, step_order: "1" },
                  { step_type: "05", step_name: "양도자", use_mail: true, use_sms: true, use_alimtalk: true, ...base, recipient: { id: customer.email_seller || "", name: customer.name_seller || "양도자" }, auth: { ...base.auth, method: "PASS" }, step_order: "1" },
                ];
              }
            ...
            const pickInput = (id) => document.querySelector(\`#main-content input[id="\${id}"]\`)?.value?.trim() || "";

            // ===== 팝업 통신 =====
            let popupRef = null, ready = false, payload = null, pingTimer = null;
            ...
            window.addEventListener("message", (event) => {
              ...
              if (data.type === "DOC_RESULT") {
                ...
                fields.forEach(f => {
                  const el = root.querySelector(\`input[id="\${CSS.escape(f.id)}"]\`);
                  if (el) el.value = (f.value ?? '');
                });
                // 필요 시 사용자 알림/저장
                console.log('[CRM] DOC_RESULT 반영 완료', data.document_id);
                alert('전자문서 완료 값이 화면에 반영되었습니다.');
              }
            });
            ...
            const openPopup = () => {
              ...
              return window.open(POPUP_URL, "efs_template", \`width=\${w},height=\${h},left=\${left},top=\${top}\`);
            };

            const resolveTemplate = ({ form, biz, contact, pageHint }) =>
              TEMPLATE_MAP.find(x => x.form === form && x.biz === biz && x.contact === contact && (!pageHint || x.page === pageHint))?.template_id || null;

            // ===== 전자문서 작성 시작 =====
            async function onClickStart(e) {
              e.preventDefault(); e.stopPropagation();

              const form = document.querySelector('#crm-form-code')?.value ||
                document.querySelector('#form-code')?.value || "APW";
              const biz = document.querySelector('#user-type')?.selectedOptions?.[0]?.textContent?.trim() || "법인사업자-대표자";
              const contactSel = document.querySelector('#contact-method')?.selectedOptions?.[0]?.textContent?.trim() || "비대면";
              ...
              if (!template_id) { alert(\`템플릿 매핑 실패: (\${form}/\${biz}/\${contactSel})\`); return; }

              popupRef = openPopup();
              if (!popupRef) { alert("팝업 차단됨. 허용해 주세요."); return; }
              ...
              try {
                ...
                // 부모가 팝업에 요구할 return_fields(주소 인터페이스 위주)
                const return_fields = ["고객주소1", "고객주소2", "고객주소3", "우편번호"];

                payload = {
                  type: "OPEN_TEMPLATE", // 안전하게 타입 명시
                ...
                pingTimer = setInterval(() => {
                  ...
                  if (++count >= 20) { clearInterval(pingTimer); pingTimer = null; alert("임베딩 준비 신호 미수신. 팝업을 새로고침하세요."); }
                }, 500);
            ...
            async function onClickManage(e) {
              ...
              try {
                ...
                alert(\`완료문서 \${res?.items?.length || res?.total || 0}건 (콘솔에서 확인)\`);
              } catch (err) {
                console.error(err);
